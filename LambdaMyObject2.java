...
class Product_data2 {
	...
	public Product_data2(String name, int price, int qty) {
		this.name = name;
		this.price = price;
		this.qty = qty;
	}
...
public class LambdaMyObject2 {
	public static void main(String[] args) {
		...
		Product_data2[] p_data_array = new Product_data2[] { new Product_data2("烈空座", 380, 23),
				new Product_data2("甲賀忍蛙", 250, 23), new Product_data2("皮卡丘", 300, 47),
				new Product_data2("雷電雲", 240, 57) };

		Arrays.sort(p_data_array, new Comparator<Product_data2>() {
			public int compare(Product_data2 product1, Product_data2 product2) {
				int p1_sum_value, p2_sum_value, temp = 1;
				p1_sum_value = product1.get_sum_value();
				p2_sum_value = product2.get_sum_value();
